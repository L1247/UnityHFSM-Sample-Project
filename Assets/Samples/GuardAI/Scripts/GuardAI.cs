...
public class GuardAI : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        var fightFsm = new HybridStateMachine(
            beforeOnLogic: _ => MoveTowards(playerPosition, attackSpeed, minDistance: 1),
            needsExitTime: true
        );
        ...
        fsm.AddTwoWayTransition("Chase", "Fight", _ => distanceToPlayer <= attackRange);
        fsm.AddTransition("Chase", "Search",
            _ => distanceToPlayer > searchSpotRange,
            onTransition: _ => lastSeenPlayerPosition = playerPosition);
